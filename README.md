...
```mermaid
graph TD
    subgraph Client ["Client-Side Private State (Voter's Enclave)"]
        SK["Voter Secret Key (sk)"]
        Salt["Random Salt (r)"]
        Choice["Ballot Choice (Yes / No / Abstain)"]
        CommitGen["Commitment = SHA256(sk || r)"]
        NullGen["Nullifier = SHA256(sk || proposalId)"]
        ZKProver["Compact ZK Prover (PLONK BLS12-381)"]
    end

    subgraph Ledger ["Midnight Public Ledger (Verifiable by All Nodes)"]
        PCount["proposalCount: Counter"]
        Proposals["proposals: Map<ProposalId, ProposalMeta>"]
        RegVoters["registeredVoters: Map<ProposalId, Map<Commitment, Boolean>>"]
        UsedNull["usedNullifiers: Map<ProposalId, Map<Nullifier, Boolean>>"]
        Tallies["Verifiable Tallies (yesCount, noCount, abstainCount)"]
    end
    ...
    ZKProver -->|"ZK Proof + Nullifier + Tally Delta"| UsedNull
    UsedNull -->|"Assert Nullifier Unused"| Tallies
    Proposals --> Tallies
```
